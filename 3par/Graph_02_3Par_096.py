...
def colour_points(variant):
    ...
    for point in variant:
        ...
        if point[0] <= xmax/2 or point[1] <= ymax/2:
            ...
            ##p_colors.append('orange')#FF4429
            ##p_colors.append('#FF4429')
...
def flatten_coef_decision(angle):
    angle_verthor_coef = abs(math.cos(math.radians(angle))) **4
...
def flatten_coef_radius(angle):
    angle_verthor_coef = abs(math.cos(math.radians(angle)))  **2
...
def occupy_points_lines(points):
    ...
    for point in points:
        occupied_points_lines.append([[point[0]-0.4,point[1]+2.5],[point[0]+0.4,point[1]-2.5]])
        occupied_points_lines.append([[point[0]-0.4,point[1]-2.5],[point[0]+0.4,point[1]+2.5]])
        occupied_points_lines.append([[point[0],point[1]-3.5],[point[0],point[1]+3.5]])
        occupied_points_lines.append([[point[0]-0.7,point[1]],[point[0]+0.7,point[1]]])
        occupied_points_lines.append([[point[0]-0.4,point[1]+2.5],[point[0]-0.4,point[1]-2.5]])
        occupied_points_lines.append([[point[0]+0.4,point[1]+2.5],[point[0]+0.4,point[1]-2.5]])
...
def save_graph(graph_name,variant_points_xy,p_colors,labels_xy,debug_lines):
    ...
    gradient = list(gradient_start.range_to(colour.Color('#DB0000'),50))
    gradient_str = []
    for g_color in gradient:
        gradient_str.append(str(g_color).strip('<Color ').strip('>'))
    for n,g_color in enumerate(gradient_str):
        ax.add_patch(patches.Rectangle(xy=(75+n/2, 100) ,width=0.5, height=100, linewidth=0, color=g_color, fill=True, alpha=0.6, zorder=1))

    # Place Labels
    point_label = 0
    for label in labels_xy:
        if variant_points_xy[point_label][0] < 75 or variant_points_xy[point_label][1] < 100:
            ax.annotate(variant_points_xy[point_label][2], (variant_points_xy[point_label][0], variant_points_xy[point_label][1]), xytext=(label[0],label[1]), fontsize=font_size, alpha=0.6, ha='center', arrowprops=dict(arrowstyle='-',color='grey', linewidth=0.8))
        else:
            ax.annotate(variant_points_xy[point_label][2], (variant_points_xy[point_label][0], variant_points_xy[point_label][1]), xytext=(label[0],label[1]), fontsize=font_size, alpha=1, ha='center', arrowprops=dict(arrowstyle='-',color='black', linewidth=0.8))
        point_label += 1
    ...
    pyplot.axhline(100,color='black',linewidth=0.7,linestyle='--' ,alpha=0.6, zorder=1)
    pyplot.axvline(75,color='black',linewidth=0.7,linestyle='--' ,alpha=0.6, zorder=1)
    # Plot tick params
    pyplot.locator_params(axis='y',nbins=4)
    pyplot.locator_params(axis='x',nbins=2)
...
def process_graph_points(i_points_xy,v,simulations_number):
    ...
    for s in range(0,simulations_number):
        ...
        for point_xy in i_points_xy:
                ...
                for angle in angles_in_between:
                    ...
                    while able_to_put != True:
                        ...
                        for occ_line in iter_occupied_lines:
                            ...
                            if check_lines_cross([[occ_line[0][0],occ_line[0][1]],[occ_line[1][0],occ_line[1][1]]],[[probe_point[0]-label_size(point_xy[2])/2,probe_point[1]],[probe_point[0]+label_size(point_xy[2])/2,probe_point[1]+5]]):
                                is_occupied_line = True
                            if check_lines_cross([[occ_line[0][0],occ_line[0][1]],[occ_line[1][0],occ_line[1][1]]],[[probe_point[0]-label_size(point_xy[2])/2,probe_point[1]+5],[probe_point[0]+label_size(point_xy[2])/2,probe_point[1]]]):
                                is_occupied_line = True
                            if check_lines_cross([[occ_line[0][0],occ_line[0][1]],[occ_line[1][0],occ_line[1][1]]],[[probe_point[0]-label_size(point_xy[2])/2,probe_point[1]+5],[probe_point[0]+label_size(point_xy[2])/2,probe_point[1]+5]]):
                                is_occupied_line = True
            ...
            iter_occupied_lines.append([[label_coordinates[0] - label_size(label_coordinates[2])/2,label_coordinates[1]+3],[label_coordinates[0] + label_size(label_coordinates[2])/2,label_coordinates[1]+3]])
            iter_occupied_lines.append([[label_coordinates[0] - label_size(label_coordinates[2])/2,label_coordinates[1]+3],[label_coordinates[0] - label_size(label_coordinates[2])/2,label_coordinates[1]]])
            iter_occupied_lines.append([[label_coordinates[0] + label_size(label_coordinates[2])/2,label_coordinates[1]+3],[label_coordinates[0] + label_size(label_coordinates[2])/2,label_coordinates[1]]])
            iter_occupied_lines.append([[label_coordinates[0] - label_size(label_coordinates[2])/2,label_coordinates[1]+3],[label_coordinates[0] - label_size(label_coordinates[2])/2,label_coordinates[1]]])
            iter_occupied_lines.append([[label_coordinates[0] + label_size(label_coordinates[2])/4,label_coordinates[1]+3],[label_coordinates[0] + label_size(label_coordinates[2])/4,label_coordinates[1]]])
            ...
            iter_occupied_lines.append([[label_coordinates[0] - label_size(label_coordinates[2])/2,label_coordinates[1]+3],[label_coordinates[0] + label_size(label_coordinates[2])/2,label_coordinates[1]]])
            iter_occupied_lines.append([[label_coordinates[0] - label_size(label_coordinates[2])/2,label_coordinates[1]],[label_coordinates[0] + label_size(label_coordinates[2])/2,label_coordinates[1]+3]])
            iter_occupied_lines.append([[label_coordinates[0],label_coordinates[1]],[label_coordinates[0],label_coordinates[1]+3]])
        ...
        save_graph('Graph_02_'+str(v+1)+str(s+1),i_points_xy,points_colors,iter_labels_xy,iter_occupied_lines+occupy_points_lines(points_xy))

    return

def produce_graph_02_variants(input_points_xy,iter_num):
    ...
    #     occupied_axis.append([75,y*4,'yaxis'])
...
# 12*10/5*19/10*6/5
char_coef = font_size/10*graph_aspect_ratio*16/8*1.3
points_distance = 11
...
if __name__ == '__main__':
    ...
    produce_graph_02_variants(input_points_xy,1)
